...
namespace AXACodingEx
{
    ...
    public class CityTree
    {
        ...
        private int MatchingConsecutiveCharacters(string word, Node curNode)
        {
            ...
            minLength = Math.Min(curNode.Label.Length, word.Length);
        ...
        private bool LookupRec(string wordPart, Node curNode)
        {
                ...
                ((matches > 0) && (matches < wordPart.Length) && (matches >= curNode.Label.Length)))
            {
                ...
                foreach (var child in curNode.SubNodes)
                    if (newLabel.Length > 0 && child.Label.StartsWith(newLabel[0].ToString()))
        ...
        public List<string> FindAllPossibleNextChars(string word)
        {
            return FindAllPossibleNextCharsRec(word, _root, string.Empty);
        }

        /// <summary>
        /// look for all the next strings and return a list of their first char
        /// </summary>
        /// <param name="wordPart"></param>
        /// <param name="curNode"></param>
        /// <returns></returns>
        public List<string> FindAllPossibleNextCharsRec(string wordPart, Node curNode, string carry)
        {
            List<string> nextChars = new List<string>();
                ...
                ((matches > 0) && (matches < wordPart.Length)))
            {
                ...
                foreach (var child in curNode.SubNodes)
                    if (newLabel.Length > 0 && child.Label.StartsWith(newLabel[0].ToString()))
                        return FindAllPossibleNextCharsRec(newLabel, child, carry + curNode.Label);

            }

            if (matches < curNode.Label.Length)
            {
                nextChars.Add(curNode.Label[matches].ToString());
                return nextChars;
            }
            ...
            else
            {
                foreach (var child in curNode.SubNodes)
                {
                    nextChars.Add(child.Label[0].ToString());
                }
                return nextChars;
            }
        ...
        public List<string> FindWordList(string word)
        {
            return FindWordListRec(word, _root, string.Empty);
        }

        /// <summary>
        /// Find all possible words starting with entered string
        /// </summary>
        /// <param name="wordPart"></param>
        /// <param name="curNode"></param>
        /// <returns></returns>
        private List<string> FindWordListRec(string wordPart, Node curNode, string carry)
        {
                ...
                ((matches > 0) && (matches < wordPart.Length)))
            {
                ...
                foreach (var child in curNode.SubNodes)
                    if (newLabel.Length > 0 && child.Label.StartsWith(newLabel[0].ToString()))
                        return FindWordListRec(newLabel, child, carry + curNode.Label);
            }

            //curNode is at the node for the entered string
            else
            {
                Node searchNode = curNode;
                MakeMatchingWordList(carry, searchNode, allMatchingWords);
            }
        ...
        public void MakeMatchingWordList(string word, Node startNode, List<string> wordList)
        {
            word += startNode.Label;

            if (startNode.SubNodes.Count >= 1)
            {
                for (int i = 0; i < startNode.SubNodes.Count; i++)
                {
                    MakeMatchingWordList(word, startNode.SubNodes[i], wordList);
                }
            }
